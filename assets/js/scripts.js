...
function show_state(color) {
	...
	if (div_extra_setting) {
		...
		var state_button = document.getElementById('state_button');
			...
			if (color == "red") {
				// state_button.innerHTML = '<img src="publicstyles/button_red.png" title="Автоподгрузке не удалось проверить наличие новых комментариев.">';
			} else if (color == "blue") {
				state_button.innerHTML = '<img src="public/img/button_blue.png" title="Идет проверка новых комментариев...">';
			} else {
				state_button.innerHTML = '<img src="public/img/button_green.png" title="Автоподгрузка новых комментариев работает нормально.">';
			}
	}
...
function preview_comment(event, comment_id) {
	...
	if (oldNode) {
		...
		if (pc_xmlhttp == null && !check_exist) {
			...
			newNode.style.top = my_curor_Y - 30 + 'px';
			newNode.style.left = event.target.parentNode.parentNode.offsetLeft + 30 + 'px';
			newNode.innerHTML = '<p><img class="text_img" src="public/styles/loading.gif" alt="" title="">На этой странице комментарий не найден.<br>Идет поиск по базе данных...</p><span class="info">Комментарий №' + comment_id + '</span>';
			document.body.appendChild(newNode);
...
function show_smiles(form_id, pack) {
	var xmlhttp = new XMLHttpRequest();
	xmlhttp.open("GET","smiles/" + form_id,true);
	smile_box = document.getElementById("smiles_" + form_id);
	smile_box.innerHTML = '<img class="text_img" src="public/img/loading.gif" alt="" title="">';
